...
import generate_random_positions_sintheta as generate_random_positions
...
#from utils_mpnn import eval_mmodel
#from utils_gen import loadpk
import torch
import pickle as pk
...
#    model = torch.load('model_' + str(icl) + '.pth')
#    models.append(model)

mpnn = pk.load(open('mpnn_model/mpnn.pk', 'rb'))

def V_pytorch(adsorbate):
    #Evaluate the surrogate
    #The model gives the relative energy (minimum energy = 0)
    kJ = adsorbate.kJ
    rhombus_coord=[adsorbate.x, adsorbate.y, adsorbate.z, adsorbate.alphar, adsorbate.betar, adsorbate.gammar] 
    ...
    while np.abs(conv_param) > adsorbate.q_accuracy:# and points_needed<3000:
        ...
        if min_dist < min_cutoff or max_dist>max_cutoff: #retranslate the ads in the plane
            ...
            if adsorbate.N_atoms == 1:
                ...
                adsorbate.gammar = ads.gamma*ads.max_r*1.0e-10
        ...
        if not valid:
            ...
            if adsorbate.N_atoms > 1 and ads.rotate==True:
                if adsorbate.N_atoms == 2:
                    ...
                    rot_kinetic_factor= 2.0*np.pi*np.sqrt(2.0*np.pi*adsorbate.inertia[0]*adsorbate.amu_to_kg*adsorbate.angstrom_to_m**2.0*kB*T_array)*2.0*np.pi*np.sqrt(2.0*np.pi*adsorbate.inertia[1]*adsorbate.amu_to_kg*adsorbate.angstrom_to_m**2.0*kB*T_array)*2.0*np.sqrt(2.0*np.pi*adsorbate.inertia[2]*adsorbate.amu_to_kg*adsorbate.angstrom_to_m**2.0*kB*T_array)/(h**3.0)
                    new_q_MC *= rot_kinetic_factor
